...
//	int project_id = 0;
	int i = 0;
	int count = sizeof(smart_pa_range_info) / sizeof(smart_pa_range_info[0]);
	if (!min_mohms || !max_mohms) {
		pr_err("null ptr to get boundary\n");
		return;
	}

	*min_mohms = SMART_PA_RANGE_DEFAULT_MIN;
	*max_mohms = SMART_PA_RANGE_DEFAULT_MAX;

//	project_id = get_project();
//	pr_info("current project = %d\n", project_id);

	for ( i = 0; i < count; i++) {
		if (is_project(smart_pa_range_info[i].prj_id) == 1) {
			*min_mohms = smart_pa_range_info[i].min;
			*max_mohms = smart_pa_range_info[i].max;
