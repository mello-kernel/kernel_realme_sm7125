...
#ifdef VENDOR_EDIT
/*chengong@ODM_LQ@Multimedia.Audio, 2019/11/06, ADD for smartpa*/
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		dev_info(&tfa98xx->i2c->dev, "%s: stream: %d\n", __func__, stream);
		if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
			tfa98xx->pstream = 1;
			if (tfa98xx->tfa->is_probus_device) {
			     ...
			     const struct i2c_device_id *id)
{
	...
	if ((no_start == 0) && (no_reset == 0)) {
		#ifdef VENDOR_EDIT
		//chengong@ODM_LQ@Multimedia.Audio,2019/12/03,Modify for chip id
		for (i = 0; i < 5; i++) {
	...
	/*chengong@ODM_LQ@Multimedia.Audio, 2019/10/22, ADD for smartpa*/
	set_smartpa_id(1);
